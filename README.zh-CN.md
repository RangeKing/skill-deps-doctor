...
**skill-deps-doctor**（包名：`skill-deps-doctor`）填补的正是这个空白。它是一个**确定性、离线、预检**工具，工作在 *skill 层*——是 `doctor` 的补充，而非替代。
...
### 🔗 生态集成
- 🐍 **编程 API**：`from skill_deps_doctor import run_check`
